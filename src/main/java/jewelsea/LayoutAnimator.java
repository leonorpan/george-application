...
public class LayoutAnimator implements ChangeListener<Number>, ListChangeListener<Node> {
  ...
  private Map<Node, MoveXTransition> nodeXTransitions = new HashMap<Node, MoveXTransition>();
  private Map<Node, MoveYTransition> nodeYTransitions = new HashMap<Node, MoveYTransition>();
  ...
  public void changed(ObservableValue<? extends Number> ov, Number oldValue, Number newValue) {
    ...
    String s = doubleProperty.getName();
    if (s.equals("layoutX")) {
  ...
  private abstract class MoveTransition extends Transition {
    private final Duration MOVEMENT_ANIMATION_DURATION = new Duration(200);
